...
rtl.module("fifteenengine",["System","Classes","SysUtils","Math"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  this.CSIZE = 15;
  ...
  this.VK_RIGHT = 102;
  this.VK_LEFT = 103;
  this.COLORBLANK = "#808080";
  this.COLOROK = "#b3ffb3";
  this.COLORWRONG = "#ff9999";
  rtl.recNewT(this,"Block",function () {
    this.n = 0;
    this.x = 0;
    this.y = 0;
    this.$eq = function (b) {
      return (this.n === b.n) && (this.x === b.x) && (this.y === b.y);
    };
    this.$assign = function (s) {
      this.n = s.n;
      this.x = s.x;
  ...
  this.Board$a$clone = function (a) {
    var b = [];
    b.length = 16;
    for (var c = 0; c < 16; c++) b[c] = $mod.Block.$clone(a[c]);
    return b;
  };
  this.Board = rtl.arraySetLength(null,this.Block,16);
  this.Blank = 0;
  this.BoardInit = function () {
    ...
    var pb = 0;
    for (pb = 0; pb <= 15; pb++) {
      pas.Math.DivMod$3(pb,4,{get: function () {
          return py;
        }, set: function (v) {
          py = v;
        }},{get: function () {
  ...
  this.Shuffle = function () {
    ...
    for (var $l = 15; $l >= 0; $l--) {
      i = $l;
      j = pas.System.Random(i);
      if (!(i === j)) {
        n = $mod.Board[i].n;
        $mod.Board[i].n = $mod.Board[j].n;
        $mod.Board[j].n = n;
      };
    };
    $impl.SetBlank();
    if (!$mod.IsSolvable()) $mod.Shuffle();
  };
  this.DoMove = function (key) {
    var Result = false;
    var d = 0;
    Result = $impl.CanMove(key);
    ...
    $mod.Board[$mod.Blank].n = $mod.Board[$mod.Blank + d].n;
    $mod.Blank = $mod.Blank + d;
    $mod.Board[$mod.Blank].n = 0;
    return Result;
  };
  this.IsSolvable = function () {
    ...
    var c = 0;
    c = 0;
    p = 4 - $mod.Board[$mod.Blank].y;
    for (i = 0; i <= 14; i++) for (var $l = i; $l <= 14; $l++) {
      j = $l;
      if (($mod.Board[i].n > 0) && ($mod.Board[j].n > 0)) if ($mod.Board[i].n > $mod.Board[j].n) c += 1;
    };
    pas.System.Writeln("c=",c," p=",p," odd(c)=",pas.System.Odd(c)," odd(p)=",pas.System.Odd(p));
    Result = pas.System.Odd(c) ^ pas.System.Odd(p);
    return Result;
  };
  $mod.$implcode = function () {
    $impl.SetBlank = function () {
      var i = 0;
      for (i = 0; i <= 15; i++) if ($mod.Board[i].n === 0) {
        $mod.Blank = i;
        pas.System.Writeln("blank=",i," x=",$mod.Board[i].x," y=",$mod.Board[i].y);
        break;
      };
    };
    $impl.CanMove = function (key) {
      var Result = false;
      Result = false;
      var $tmp = key;
      if ($tmp === 100) {
        if ($mod.Board[$mod.Blank].y === 3) return Result}
       else if ($tmp === 101) {
        if ($mod.Board[$mod.Blank].y === 0) return Result}
       else if ($tmp === 102) {
        if ($mod.Board[$mod.Blank].x === 0) return Result}
       else if ($tmp === 103) if ($mod.Board[$mod.Blank].x === 3) return Result;
      Result = true;
      return Result;
    };
  };
},[]);
rtl.module("program",["System","BrowserApp","JS","Classes","SysUtils","Web","fifteenengine"],function () {
  "use strict";
  var $mod = this;
  rtl.createClass(this,"TMyApplication",pas.BrowserApp.TBrowserApplication,function () {
    ...
    this.BindElements = function () {
      ...
      this.FCtx.font = "bold 40px Arial";
      this.FCtx.textAlign = "center";
      document.onkeydown = rtl.createSafeCallback(this,"OnFormKeyPress");
      pas.fifteenengine.BoardInit();
      this.DrawBoard();
    };
    this.OnFormKeyPress = function (k) {
      var Result = false;
      var key = 0;
      Result = true;
      var $tmp = k.code;
      if ($tmp === pas.Web.TJSKeyNames.ArrowRight) {
        key = 102}
       else if ($tmp === pas.Web.TJSKeyNames.ArrowUp) {
        key = 100}
       else if ($tmp === pas.Web.TJSKeyNames.ArrowLeft) {
        key = 103}
       else if ($tmp === pas.Web.TJSKeyNames.ArrowDown) key = 101;
      k.preventDefault();
      if (pas.fifteenengine.DoMove(key)) this.DrawBoard();
      return Result;
    };
    this.NewGame = function (aEvent) {
      var Result = false;
      pas.fifteenengine.Shuffle();
      this.DrawBoard();
      return Result;
    };
    this.DrawBoard = function () {
      ...
      for (i = 0; i <= 15; i++) {
        this.FCtx.beginPath();
        x = pas.fifteenengine.Board[i].x;
        x = (x * 100) + (x * 4) + 4;
        y = pas.fifteenengine.Board[i].y;
        y = (y * 100) + (y * 4) + 4;
        this.FCtx.strokeStyle = "Black";
        if (pas.fifteenengine.Board[i].n === 0) {
          this.FCtx.fillStyle = pas.fifteenengine.COLORBLANK;
          this.FCtx.rect(x,y,100,100);
          this.FCtx.fillRect(x,y,100,100);
          this.FCtx.stroke();
          continue;
        } else {
          if (pas.fifteenengine.Board[i].n === (i + 1)) {
            this.FCtx.fillStyle = pas.fifteenengine.COLOROK}
           else this.FCtx.fillStyle = pas.fifteenengine.COLORWRONG;
          this.FCtx.rect(x,y,100,100);
          this.FCtx.fillRect(x,y,100,100);
          this.FCtx.fillStyle = "Black";
          this.FCtx.fillText(pas.SysUtils.TIntegerHelper.ToString$1.call({p: pas.fifteenengine.Board[i], get: function () {
              return this.p.n;
            }, set: function (v) {
              this.p.n = v;
            }}),x + rtl.trunc(100 / 2),y + rtl.trunc(100 / 2));
          this.FCtx.stroke();
        };
      };
    };
    ...
    var $r = this.$rtti;
    $r.addField("btNew",pas.Web.$rtti["TJSHTMLButtonElement"]);
    $r.addField("FCanvas",pas.Web.$rtti["TJSHTMLCanvasElement"]);
    $r.addField("FCtx",pas.Web.$rtti["TJSCanvasRenderingContext2D"]);
    $r.addMethod("BindElements",0,[]);
    $r.addMethod("OnFormKeyPress",1,[["k",pas.Web.$rtti["TJSKeyboardEvent"]]],rtl.boolean);
    $r.addMethod("NewGame",1,[["aEvent",pas.Web.$rtti["TJSMouseEvent"]]],rtl.boolean);
    $r.addMethod("DrawBoard",0,[]);
  });
...
//# sourceMappingURL=quinze_web.js.map
